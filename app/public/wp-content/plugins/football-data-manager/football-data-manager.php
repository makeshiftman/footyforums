...
// Load ingest runner (needed for cron and WP-CLI)
require_once __DIR__ . '/includes/ingest/ingest-runner.php';
// Load Platinum Master Datasource Engine
require_once __DIR__ . '/includes/class-fdm-e-master-datasource.php';
// Load Daily Updater
require_once __DIR__ . '/includes/class-fdm-daily-updater.php';
// Load Scheduler
require_once __DIR__ . '/includes/class-fdm-scheduler.php';
new FDM_Scheduler();

if ( defined( 'WP_CLI' ) && WP_CLI ) {
    require_once __DIR__ . '/includes/wp-cli-commands.php';
}
...
// Schedule ingest cron on activation
register_activation_hook( __FILE__, function() {
    if ( ! wp_next_scheduled( 'fdm_ingest_cron_tick' ) ) {
        wp_schedule_event( time() + 60, 'fdm_5min', 'fdm_ingest_cron_tick' );
    }
} );

// Clear ingest cron on deactivation
register_deactivation_hook( __FILE__, function() {
    wp_clear_scheduled_hook( 'fdm_ingest_cron_tick' );
} );

// Cron callback
add_action( 'fdm_ingest_cron_tick', function() {
    fdm_ingest_run_due_once( 600 );
} );
...
/**
 * Get current Football Data admin tab.
 *
 * Tabs: dashboard, e_datasource, settings, logs, tools
 */
function fdm_get_current_admin_tab() {
    $allowed = array( 'dashboard', 'e_datasource', 'settings', 'logs', 'tools' );
...
/**
 * Render tab navigation for the Football Data admin page.
 */
function fdm_render_admin_tabs() {
    $current = fdm_get_current_admin_tab();

    $tabs = array(
        'dashboard'   => 'Dashboard',
        'e_datasource'=> 'E Datasource',
        'settings'    => 'Settings',
        'logs'        => 'Logs',
        'tools'       => 'Tools',
    );

    $base_url = admin_url( 'admin.php?page=football-data-manager' );
 ...
 */
function fdm_render_dashboard_tab() {
    ?>
 ...
 */
function fdm_render_settings_tab() {
    if ( function_exists( 'fdm_render_settings_page' ) ) {
        fdm_render_settings_page();
    } else {
 ...
 */
function fdm_render_logs_tab() {
    require_once FDM_PLUGIN_DIR . 'includes/db-helper.php';
    $db = fdm_get_footyforums_db();
    
    if ( ! $db ) {
        echo '<div class="notice notice-error"><p>Cannot connect to external database to view logs.</p></div>';
        return;
    }
    ...
    $recent_logs = $db->get_results(
        "SELECT log_level, log_message, context_data, created_at 
         FROM datasource_log 
         ORDER BY created_at DESC 
         LIMIT 50",
        ARRAY_A
                    ...
                    <tbody>
                        ...
                        ?>
                            <tr>
                                ...
                                <td><strong><?php echo esc_html( $error['error_type'] ); ?></strong></td>
                                <td><?php echo esc_html( $error['error_message'] ); ?></td>
                                <td><?php echo $context_str; ?></td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
    ...
    <div class="postbox" style="margin-top: 20px;">
        <div class="inside">
            <h3>Recent Log Entries (last 50)</h3>
                ...
                <table class="widefat">
                    ...
                    <tbody>
                        <?php foreach ( $recent_logs as $log ) : 
                            $context = ! empty( $log['context_data'] ) ? json_decode( $log['context_data'], true ) : array();
                            $context_str = ! empty( $context ) ? '<pre style="font-size: 11px; max-width: 300px; overflow: auto;">' . esc_html( print_r( $context, true ) ) . '</pre>' : '-';
                        ?>
                            <tr>
                                <td><?php echo esc_html( $log['created_at'] ); ?></td>
                                <td><strong><?php echo esc_html( $log['log_level'] ); ?></strong></td>
 ...
 */
function fdm_render_tools_tab() {
    ?>
    <h2>Utilities and Maintenance Tools</h2>
    <p>Database maintenance and utility tools will go here.</p>
    <?php
}

/**
 * Render legacy tab content
 * Contains old ktpl_* import UI
 */
function fdm_render_legacy_tab() {
    if ( function_exists( 'fdm_render_e_imports_page' ) ) {
        fdm_render_e_imports_page();
    } else {
        echo '<p>Legacy import UI and old tools will go here temporarily.</p>';
    }
}
...
function fdm_admin_page() {
    ...
    $tab = fdm_get_current_admin_tab();
    
    echo '<div class="wrap">';
    echo '<h1>Football Data Manager</h1>';
    
    // Tab navigation
    fdm_render_admin_tabs();
    
    echo '<div class="fdm-tab-content">';
    
    switch ( $tab ) {
        case 'dashboard':
            if ( function_exists( 'fdm_render_dashboard_tab' ) ) {
                fdm_render_dashboard_tab();
            } else {
                echo '<p>Dashboard overview will go here.</p>';
            }
            break;
            
        case 'e_datasource':
            if ( function_exists( 'fdm_render_e_datasource_tab' ) ) {
                fdm_render_e_datasource_tab();
            } else {
                echo '<p>E Datasource tools will go here.</p>';
            }
            break;
            
        case 'settings':
            if ( function_exists( 'fdm_render_settings_tab' ) ) {
                fdm_render_settings_tab();
            } else {
                echo '<p>Settings form will go here.</p>';
            }
            break;
        ...
        case 'logs':
            if ( function_exists( 'fdm_render_logs_tab' ) ) {
                fdm_render_logs_tab();
            } else {
                echo '<p>Import and error logs will go here.</p>';
            }
            break;
            
        case 'tools':
            if ( function_exists( 'fdm_render_tools_tab' ) ) {
                fdm_render_tools_tab();
            } else {
                echo '<p>Utilities and maintenance tools will go here.</p>';
            }
            break;
    }
    
    echo '</div>'; // .fdm-tab-content
    echo '</div>'; // .wrap
}

/**
 * Render E Datasource tab content
 * Contains the E Datasource import forms and full season import
 * This extracts the content from the old 'imports' case
 */
function fdm_render_e_datasource_tab() {
    // Use the old 'imports' case content
    $current_tab = 'imports'; // Keep compatibility with existing code that checks this
                    ?>
                    <h2>Data Imports</h2>
...
function fdm_render_e_imports_page() {
    ...
    // Load the datasource trait (if file exists)
    $datasource_file = FDM_PLUGIN_DIR . 'includes/e_datasource.php';
    if ( ! file_exists( $datasource_file ) ) {
        echo '<div class="notice notice-warning"><p><strong>Legacy functionality unavailable:</strong> The file <code>includes/e_datasource.php</code> is missing. This legacy UI requires the old datasource trait file.</p>';
        echo '<p>This functionality has been replaced by the new E Datasource tab. Please use the <a href="' . esc_url( admin_url( 'admin.php?page=football-data-manager&tab=e_datasource' ) ) . '">E Datasource</a> tab for importing data.</p></div>';
        return;
    }
